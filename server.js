...
io.on("connection", (socket) => {
    ...
    socket.on("please give lobbies", () => {
        ...
        dbUtil.getLobbies().then((lobbies) => {
    ...
    socket.on("disconnect", () => {
        ...
        socket_info.email && socket_info.lobby
            ? deletePlayerFromRoom({ room: socket_info.lobby, email: socket_info.email })
            : true;
...
function deletePlayerFromRoom(info) {
    ...
    io.emit("update lobby list", rooms_playerlist[info.room]);
}
